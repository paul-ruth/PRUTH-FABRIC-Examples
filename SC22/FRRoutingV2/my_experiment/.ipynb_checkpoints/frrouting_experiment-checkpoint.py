...
class FRRouting_Experiment():
    ...
    def configure(self, frr_sites, frr_links):
        ...
        try:
            ...
            for name, data in frr_sites.items():
                routers[name] = self.add_router(name=f'router_{name}', site=data['site'], cores=32, ram=128, disk=10)
    ...
    def upload_directory_to_all_nodes(self, directory, verbose=False):
        ...
        for router in self.get_routers():
            threads.append(router.upload_directory_thread(directory, '.'))
    ...
    def execute_on_all_nodes(self, command, verbose=False):
        ...
        for router in self.get_routers():
            log_file = f"logs/{router.get_name()}.log"
